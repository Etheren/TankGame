...
class Design_38_38_EnemyFireProjectile extends ActorScript
{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(!(_Firing))
				{
					_Firing = true;
					propertyChanged("_Firing", _Firing);
					createRecycledActor(getActorType(39), (actor.getXCenter() - 20), (actor.getYCenter() - 28), Script.BACK);
					...
					getLastCreatedActor().setVelocity(Utils.DEG * (Math.atan2(((Engine.engine.getGameAttribute("PlayerY") + 30) - actor.getY()), ((Engine.engine.getGameAttribute("PlayerX") + 15) - actor.getX()))), _FiringSpeed);
					runLater(1000 * _Delay, function(timeTask:TimedTask):Void
					{
						_Firing = false;
						propertyChanged("_Firing", _Firing);
